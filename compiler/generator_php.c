...
static void write_varname(struct generator * g, struct name * p) {
    ...
    if (p->type != t_routine && p->type != t_external) {
      if (g->writing_class_vars) {
          w(g, "$");
      } else {
          w(g, "$this->");
      }
...
static void write_inc_cursor(struct generator * g, struct node * p) {
    ...
    write_margin(g);
    write_string(g, p->mode == m_forward ? "$this->cursor += 1;" : "$this->cursor -= 1;");
    write_newline(g);
}
...
static void writef(struct generator * g, const char * input, struct node * p) {
    ...
    while (i < l) {
        ...
        switch (input[i++]) {
            ...
            //case 'F': write_funcname(g, g->V[input[i++] - '0']); continue;
...
static void generate_backwards(struct generator * g, struct node * p) {
    ...
    generate(g, p->left);
    w(g, "~M$this->cursor = $this->limit_backward;");
}
...
static void generate_tolimit(struct generator * g, struct node * p) {
    ...
    writef(g, "~M$this->cursor = $this->~S0;~N", p);
}
...
static void generate_atlimit(struct generator * g, struct node * p) {
    ...
    write_failure_if(g, "$this->cursor ~S1 $this->~S0", p);
}
...
static void generate_leftslice(struct generator * g, struct node * p) {
    ...
    writef(g, "~M$this->~S0 = $this->cursor;~N", p);
}
...
static void generate_sliceto(struct generator * g, struct node * p) {
    ...
    write_comment(g, p);
    g->V[0] = p->name;
    writef(g, "~M~V0 = $this->slice_to(~V0);~N", p);
}
...
static void generate_insert(struct generator * g, struct node * p, int style) {
    ...
    if (keep_c) w(g, "~{~M$c = $this->cursor;~N");
...
static void generate_assignfrom(struct generator * g, struct node * p) {
    ...
    write_comment(g, p);
    if (keep_c) writef(g, "~{~M$c = $this->cursor;~N", p);
    if (p->mode == m_forward) {
...
static void generate_dollar(struct generator * g, struct node * p) {
    ...
    writef(g, "~{~M~n ~B0 = this;~N"
             "~M$this->current = new StringBuffer(~V0->__toString());~N"
             "~M$this->cursor = 0;~N"
             "~M$this->limit = ($this->current->length());~N", p);
    generate(g, p->left);
    if (!g->unreachable) {
        write_margin(g);
        write_str(g, g->failure_str);
...
static void generate_file_header(struct generator * g) {
...
static void generate_start_comment(struct generator * g) {
    ...
    w(g, "/* ~N * This file was generated automatically by the Snowball to PHP compiler~N");
...
static void generate_class_begin(struct generator * g) {
    ...
    w(g,
         "~N"
         " /**~N"
         "  * This class was automatically generated by a Snowball to PHP compiler~N"
...
static void generate_among_declare(struct generator * g, struct among * x) {
...
static void generate_amongs_declares(struct generator * g) {

    struct among * x = g->analyser->amongs;
    while (x != 0) {
        generate_among_declare(g, x);
        x = x->next;
    }
}
...
static void generate_constructor(struct generator * g) {
    ...
    generate_amongs_declares(g);

    w(g, "~Mpublic function __construct() {~N~+");
    w(g, "~Mparent::__construct();~N~N");
...
static void generate_grouping_table(struct generator * g, struct grouping * q) {
    ...
    g->V[0] = q->name;
    g->writing_class_vars = 1;

    w(g, "~M/** @var int[] */~N");
    w(g, "~Mprivate ~V0 = array(");
    for (i = 0; i < size; i++) {
        write_int(g, map[i]);
        if (i < size - 1) w(g, ", ");
    }
...
static void generate_copyfrom(struct generator * g) {
    ...
    w(g, "~Mprivate function copy_from(~n $other) {~+~N");
